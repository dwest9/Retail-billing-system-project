from dataclasses import dataclass
from datetime import datetime
import sqlite3
from typing import Any, Optional
import functools


@dataclass
class Item:
    id: int
    name: str
    price: float
    category: int
    gst: bool
    pst: bool

    @staticmethod
    def from_row(row: Any):
        id, name, price, gst, pst, categoryID = row
        return Item(id, name, price, categoryID, gst, pst)


@dataclass
class ItemQuantity(Item):
    quantity: int

    @staticmethod
    def from_row(row: Any):
        id, quantity, name, price, gst, pst, categoryID = row
        return ItemQuantity(
            id=id,
            quantity=quantity,
            name=name,
            price=price,
            category=categoryID,
            gst=gst,
            pst=pst,
        )

    def subtotal(self):
        return self.price * self.quantity

    def get_gst(self):
        return self.price * self.quantity * int(self.gst) * 0.05

    def get_pst(self):
        return self.price * self.quantity * int(self.pst) * 0.06

    def total(self):
        return self.subtotal() + self.get_gst() + self.get_pst()


@dataclass
class Order:
    id: int
    customer_id: int
    user_id: int
    payment: Optional[int]
    timestamp: str
    items: list[ItemQuantity]

    @staticmethod
    def from_row(row: Any):
        id, customer_id, user_id, payment_type, timestamp = row
        return Order(id, customer_id, user_id, payment_type, timestamp, [])

    def calculate_subtotal(self):
        return functools.reduce(lambda a, b: a + b.subtotal(), self.items, 0)

    def calculate_gst(self):
        return functools.reduce(lambda a, b: a + b.get_gst(), self.items, 0)

    def calculate_pst(self):
        return functools.reduce(lambda a, b: a + b.get_pst(), self.items, 0)

    def calculate_total(self):
        return functools.reduce(lambda a, b: a + b.total(), self.items, 0)

    def to_string(self) -> str:
        string = (
            f"Bill Number: {self.id}\n"
            + f"Customer ID: {self.customer_id}\n"
            + f"User ID: {self.user_id}\n"
            + f"Payment: {self.payment}\n"
            + f"Timestamp: {self.timestamp}\n"
        )
        for item in self.items:
            string += f" - {item.name}  x{item.quantity}  ${item.subtotal():.2f}\n"
        string += f"Subtotal: {self.calculate_subtotal():.2f}\n"
        string += f"GST: {self.calculate_gst():.2f}\n"
        string += f"PST: {self.calculate_pst():.2f}\n"
        string += f"Total: {self.calculate_total():.2f}"
        return string


@dataclass
class OrderSummary:
    id: int
    customer_id: Optional[int]
    user_id: int
    timestamp: str
    num_items: int
    subtotal: float
    gst_total: float
    pst_total: float

    @staticmethod
    def from_row(row: Any):
        (
            id,
            customer_id,
            user_id,
            timestamp,
            num_items,
            subtotal,
            gst_total,
            pst_total,
        ) = row
        return OrderSummary(
            id,
            customer_id,
            user_id,
            timestamp,
            num_items,
            subtotal,
            gst_total,
            pst_total,
        )


@dataclass
class Customer:
    id: int
    name: str
    phone_number: Optional[str]
    email: Optional[str]

    @staticmethod
    def from_row(row: Any):
        id, name, phone_number, email = row
        return Customer(id, name, phone_number, email)


@dataclass
class User:
    id: int
    username: int
    is_manager: bool

    @staticmethod
    def from_row(row: Any):
        id, username, _, is_manager = row
        return User(id, username, is_manager)


class OrderSystem:
    """Order System class"""

    def __init__(self, conn: sqlite3.Connection) -> None:
        self.conn = conn

    def new_order(self, user_id: int, customer_id: Optional[int] = None) -> int:
        """Create a new order

        Args:
            user_id (int): id of user who creates the order
            customer_id (Optional[int], optional): id of customer. Defaults to None.

        Raises:
            RuntimeError: if db did not set last row id

        Returns:
            int: id of the new order
        """
        cur = self.conn.execute(
            "INSERT INTO orders(customer_id, user_id) VALUES (?,?);",
            (customer_id, user_id),
        )
        self.conn.commit()
        if cur.lastrowid is None:
            raise RuntimeError
        return cur.lastrowid

    def add_customer_to_order(self, order_id: int, customer_id: int):
        """Link a customer to an order

        Args:
            order_id (int): id of order
            customer_id (int): id of customer
        """
        self.conn.execute(
            "UPDATE orders SET customer_id = ? WHERE id = ?;", (customer_id, order_id)
        )
        self.conn.commit()

    def pay_for_order(self, order_id: int, payment_type: int):
        """Marks an order as paid for

        Args:
            order_id (int): id of order to pay for
            payment_type (int): id of payment type
        """
        self.conn.execute(
            "UPDATE orders SET payment_type = ? WHERE id = ?;",
            (payment_type, order_id),
        )
        self.conn.commit()

    def order_paid(self, order_id: int) -> bool:
        """Checks if an order is marked paid

        Args:
            order_id (int): id of order to check

        Returns:
            bool: true if paid, otherwise false
        """
        cur = self.conn.execute(
            "SELECT payment_type FROM orders WHERE id = ?;", (order_id,)
        )
        return cur.fetchone()[0] is not None

    def add_order_item(self, order_id: int, item_id: int):
        """Add item to an order

        Args:
            order_id (int): order to add item to
            item_id (int): id of item to add
        """
        self.conn.execute(
            "INSERT INTO order_items(order_id, item_id, quantity) VALUES (?, ?, ?) ON CONFLICT(order_id, item_id) DO UPDATE SET quantity = quantity + 1;",
            (order_id, item_id, 1),
        )
        self.conn.commit()

    def remove_order_item(self, order_id: int, item_id: int):
        """Remove an item from the order

        Args:
            order_id (int): order to remove item from
            item_id (int): id of item to remove
        """
        self.conn.execute(
            "UPDATE order_items SET quantity = max(quantity - 1, 0) WHERE order_id = ? AND item_id = ?;",
            (order_id, item_id),
        )
        self.conn.commit()

    def get_all_orders(self):
        cur = self.conn.execute("SELECT * FROM orders;")
        return cur.fetchall()

    def get_order_details(self, order_id: int) -> Order:
        """Get details of an order

        Args:
            order_id (int): id of order to get

        Returns:
            Order: details of the order
        """
        cur = self.conn.execute("SELECT * FROM orders WHERE id = ?;", (order_id,))
        cur2 = self.conn.execute(
            "SELECT items.id, quantity, name, price, gst, pst, category_id FROM order_items LEFT JOIN items ON order_items.item_id = items.id WHERE order_items.order_id = ?;",
            (order_id,),
        )
        order = Order.from_row(cur.fetchone())
        order.items = list(map(ItemQuantity.from_row, cur2.fetchall()))
        return order

    def create_customer(
        self,
        customer_name: str,
        customer_phone: Optional[str] = None,
        customer_email: Optional[str] = None,
    ) -> int:
        """Create a new customer

        Args:
            customer_name (str): Name of the customer
            customer_phone (Optional[str], optional): phone number of the customer. Defaults to None.
            customer_email (Optional[str], optional): email of the customer. Defaults to None.

        Raises:
            RuntimeError: if db did not set last row id

        Returns:
            int: id of the new customer
        """
        cur = self.conn.execute(
            "INSERT INTO customers(customer_name, phone_number, email) VALUES(?, ?, ?);",
            (customer_name, customer_phone, customer_email),
        )
        self.conn.commit()
        if cur.lastrowid is None:
            raise RuntimeError
        return cur.lastrowid

    def get_all_customers(self) -> list[Customer]:
        """Get all customers from the database

        Returns:
            list[Customer]: list of customers from the database
        """
        cur = self.conn.execute("SELECT * FROM customers;")
        return list(map(Customer.from_row, cur.fetchall()))

    def search_customer_by_name(self, customer_name: str) -> list[Customer]:
        """Search for customers by name

        Args:
            customer_name (str): name or partial name

        Returns:
            list[Customer]: list of customers based on the name
        """
        cur = self.conn.execute(
            "SELECT * FROM customers WHERE customer_name LIKE '%' || ? || '%';",
            (customer_name,),
        )
        return list(map(Customer.from_row, cur.fetchall()))

    def search_customer_by_email(self, customer_email: str) -> list[Customer]:
        """Search for customers by email

        Args:
            customer_email (str): email or partial email

        Returns:
            list[Customer]: list of customers based on the email
        """
        cur = self.conn.execute(
            "SELECT * FROM customers WHERE email LIKE '%' || ? || '%';",
            (customer_email,),
        )
        return list(map(Customer.from_row, cur.fetchall()))

    def search_customer_by_phone_number(self, phone_number: str) -> list[Customer]:
        """Search for customers by phone number

        Args:
            phone_number (str): phone number or partial phone number

        Returns:
            list[Customer]: list of customers based on phone number
        """
        cur = self.conn.execute(
            "SELECT * FROM customers WHERE phone_number LIKE '%' || ? || '%';",
            (phone_number,),
        )
        return list(map(Customer.from_row, cur.fetchall()))

    def get_all_order_items_raw(self):
        cur = self.conn.execute(
            "SELECT * FROM order_items LEFT JOIN items ON order_items.item_id = items.id;"
        )
        return cur.fetchall()

    def get_all_order_items(self):
        cur = self.conn.execute(
            "SELECT order_items.order_id, order_items.item_id, items.name, items.category_id, items.price, items.price * items.gst * 0.05, items.price * items.pst * 0.06  FROM order_items LEFT JOIN items ON order_items.item_id = items.id;"
        )
        return cur.fetchall()

    def get_all_order_summaries(self) -> list[OrderSummary]:
        """Get all order summaries from the database

        Returns:
            list[OrderSummary]: list of all order summaries
        """
        cur = self.conn.execute("SELECT * FROM order_summary;")
        return list(map(OrderSummary.from_row, cur.fetchall()))

    def get_order_summary_by_order_id(self, order_id: int) -> OrderSummary:
        """Get order summary matching a specific id

        Args:
            order_id (int): id of order

        Returns:
            OrderSummary: OrderSummary from the order id
        """
        cur = self.conn.execute(
            "SELECT * FROM order_summary WHERE id = ?;", (order_id,)
        )
        return OrderSummary.from_row(cur.fetchone())

    def get_order_summaries_by_customer_id(
        self, customer_id: int
    ) -> list[OrderSummary]:
        """Get order summaries matching a customer id

        Args:
            customer_id (int): id of customer

        Returns:
            list[OrderSummary]: list of all order summaries
        """

        cur = self.conn.execute(
            "SELECT * FROM order_summary WHERE customer_id = ?;", (customer_id,)
        )
        return list(map(OrderSummary.from_row, cur.fetchall()))

    def get_orders_by_date_range(
        self, start: datetime, end: datetime
    ) -> list[OrderSummary]:
        """Get order summaries within a date range

        Args:
            start (datetime): start of date range
            end (datetime): end of date range

        Returns:
            list[OrderSummary]: list of order summaries within the date range
        """
        cur = self.conn.execute(
            "SELECT * FROM order_summary WHERE timestamp BETWEEN ? AND ?;", (start, end)
        )
        return list(map(OrderSummary.from_row, cur.fetchall()))
